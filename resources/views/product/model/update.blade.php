                                    ...
                                    <div class="form-row">
                                        <div class="col-md-12">
                                            <div class="form-group m-0">
                                                <label for="name" class="col-form-label s-12">Product Model <span class="required">*</span></label>
                                                <input name="name"  id="category" placeholder="Enter Model Name" class="form-control-lg r-0 light s-12 input1" type="text" value="{{$master->name}}">
                                            </div>
...
@section('data')
    <script type="text/javascript">
        ...
        $("#category").on("input", function(e) {
            ...
            $.ajax({
                /* location to go*/
                url: "{{route('vehicle.Model_name_validate')}}",
                method: "POST",
                dataType: 'json',
                data: {
                    'name':name,
...
//            alert('hi');
            $.ajax({
                /* location to go*/
                url: "{{route('save_update.product_model')}}",
                method: "POST",
                dataType: 'json',
                data: form,
                processData: false,
                ...
                success: function(data){
                    ...
                    if(data.verify=='true'){
                        window.location.href='{{url('/product-models')}}';
                    }
                },
                error: function(er){}
            });
