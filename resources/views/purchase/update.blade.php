...
@section('content')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-3">
                                            <div class="form-group">
                                                <label for="customer">Supplier <span class="required">*</span></label>
                                                <select id="supplier" name="supplier" class="custom-select select2 tags input1" >
                                                    <option value="">Select None</option>
                                                    @foreach($sup as $s)
                                                        @if($s->id==$master->supplier_id)
                                        ...
                                        <div class="col-md-4 mb-3">
                                            <div class="form-group">
                                                ...
                                                <div class="input-group date date_picker" data-provide="datepicker">
                                                    ...
                                                    <input id="date" type="text" class="form-control input2" placeholder="Select None" name="date" value="{{date('d/M/Y', strtotime($master->date))}}">
                                        ...
                                        <div class="col-md-12 mb-3">
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control input-address hideSearch input7"
                                                          placeholder="Purchase Note" rows="2" name="note" >{{$master->note}}</textarea>
                                                <span class="text-danger">
    ...
    <input type="hidden" id="total_amount" value="{{$master->total_amount}}">
...
@section('data')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            //display detail data
            var url_req="{{url('purchase_detail/update')}}/"+$('#master_id').val();
...
//                                            console.log(data);
                    ...
                    for (var i = 0; i < data.detail.length; i++) {
                        ...
                        var total="$" + parseFloat(data.detail[i].total_cost).format(2);
                         ...
                         */
                        var des_field="<td class='font-btb'> <select disabled name='description' class='custom-select select2 inv-des" + id + "'' > <option value=''>Select None</option> </select> </td>";
                        ...
                        //selected the update product
                        /*for (var j=0;j<data.purchase_product.length;j++){
                         //alert(data.detail[i].pro_id +','+ data.purchase_product[j].id);
                         if (data.detail[i].pro_id==data.purchase_product[j].id){
                         option+='<option selected value="'+data.purchase_product[j].id+'">'+data.purchase_product[j].name +'</option>';
                         break;
                         }
                         }*/
                        ...
                        if (i==data.detail.length-1){
                            ...
                            option+='<option value="">Select None</option>';
                            for (var j=0;j<data.purchase_product.length;j++) {
                                option += '<option value="' + data.purchase_product[j].id + '">' + data.purchase_product[j].name + '</option>';
                            }
                        }
                    }
                }
...
//                        $("#form-product")[0].reset();
                        ...
                        for(var i=1;i<rowCount+1;i++) {
                            ...
                            $.ajax({
                                async: false,
                                type: 'post',
                                url: "{{route('save_update_detail.purchase')}}",
                                data: {
                                    "_token":"<?=csrf_token()?>",
                                    'qty': qty,
