                    ...
                    <div class="card no-b  ">
                        <div class="card-body">
                            <div class="form-row">
                                <div class="col-md-6">
                                    <div class="form-group m-0">
                                        ...
                                        <input name="name"  id="category" placeholder="Enter Role Name" class="form-control-lg r-0 light s-12 input1" type="text" value="{{$master->name}}">
                                    </div>
                                                    ...
                                                    @if($m->sub->count()>1)
                                                        <li class="list-group-item">
                                                            ...
                                                            <div class="material-switch float-right">
                                                                {{--Check if all sub module is checked--}}
                                                                @php($n=0)
                                                                @foreach($m->sub as $s)
                                                                    @foreach($master->pri as $pri)
                                                                        @if($pri->sub_module_id==$s->id)
                                                                            @php($n++)
                                                                            @break($pri)
                                                                        @endif
                                                                    @endforeach
                                                                ...
                                                                {{--Checked if sub module checked all--}}
                                                                @if($n==$m->sub->count())
                                                                    <input id="all{{$m->id}}" name="all" type="checkbox" checked/>
                                                                @else
                                                ...
                                                <ul class="list-group">
                                                    ...
                                                    @foreach($m->sub as $s)
                                                        <li class="list-group-item" style="text-transform: capitalize">
                                                            ...
                                                            <div class="material-switch float-right">
                                                                {{--check privilege if the same sub module checked--}}
                                                                @php($n=0)
                                                                @foreach($master->pri as $pri)
                                                                    @if($pri->sub_module_id==$s->id)
                                                                        @php($n=1)
                                                                ...
                                                                {{--Checked if has the same sub module with privilege--}}
                                                                @if($n==1)
                                                                    <input id="sub{{$s->id}}" type="checkbox" checked/>
                                                                    @php($n=0)
                                                                @else
                            ...
                            <div style="text-align: right">
                                <button type="button" class="btn btn-default btn-lg btn-close" style="margin: 18px 13px 15px 0px;" onclick="window.location.href='{{url(config('global.index_link'))}}'">Close</button>
